...
from brevitas.graph.equalize import GraphRotationEqualization, MergeLnAffine, _batch_norm
...
def test_models(rotation_fixtures):
    ...
    inp = torch.ones(in_shape).cuda() 

    model.eval()
    weight = copy.deepcopy(list(iter(model.parameters()))[-2])
    with torch.no_grad():
        expected_out = model(inp)
