...
class gpxq_mode(quantization_status_manager):
    ...
    def __init__(
        ...
        if functional_state is not None and not inplace:
            raise ValueError('Functional GPxQ requires inplace=True because targets own model parameters.')
        if not inplace:
    ...
    def update(self):
        ...
        if active_target is not None:
            ...
            if optimizer.nsamples < self.min_samples or optimizer.nsamples == 0:
                self._finish_functional_target(active_target, optimizer, 'insufficient calibration samples')
            else:
    ...
    def _finish_functional_target(
        ...
        if reason is not None:
            if self.insufficient_samples == 'error':
                raise RuntimeError(f'Functional GPxQ target {target.name} has {optimizer.nsamples} samples.')
    ...
    def _advance_functional_target(self) -> None:
        ...
        self.active_functional_target = next(
            (target for target in self.functional_targets
             if target.name not in self.completed_functional_targets), None)
...
class GPxQ(ABC):
    ...
    def __init__(
        ...
        if create_weight_orig and not isinstance(self.layer, FunctionalLinearTarget) and not hasattr(
                self.layer, 'weight_orig'):
