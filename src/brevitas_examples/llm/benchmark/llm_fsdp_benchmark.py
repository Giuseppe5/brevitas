...
# ---------------------------------------------------------------------------
# Module-level configuration — set by __main__ before benchmark() is called.
# ---------------------------------------------------------------------------

#: Path to the accelerate config YAML.  When ``None``, ``accelerate launch``
#: uses its built-in default (``~/.cache/huggingface/accelerate/default_config.yaml``).
_accelerate_config_file: Optional[str] = None

#: When ``True``, ``--quiet`` is forwarded to ``accelerate launch`` to
#: suppress its startup banner.
_accelerate_quiet: bool = False
...
class LLMFSDBenchmarkUtils(BenchmarkUtils):
    ...
    def parse_log(job_log: str) -> Dict[str, Any]:
        return LLMBenchmarkUtils.parse_log(job_log)
    ...
    def entrypoint_main(
        ...
        argv = _namespace_to_argv(
            args, LLMFSDBenchmarkUtils.argument_parser, skip_keys={"job_folder", "config"})
        if extra_args:
        ...
        # Build the accelerate launch command.  When
        # _accelerate_config_file is None, accelerate uses its built-in
        # default path (~/.cache/huggingface/accelerate/default_config.yaml).
        cmd = [sys.executable, "-m", "accelerate.commands.launch"]
        if _accelerate_config_file is not None:
            cmd += ["--config_file", _accelerate_config_file]
        if _accelerate_quiet:
            cmd.append("--quiet")
        cmd += [
            "--num_processes",
            str(nproc),
...
if __name__ == "__main__":
    ...
    _accelerate_config_file = _pre_args.accelerate_config
    _accelerate_quiet = _pre_args.quiet

    benchmark(LLMFSDBenchmarkUtils, _remaining)
